...
(function($) {
	...
	function RoyalSlider(element, options) {
		...
		self.slider.children().detach().each(function() {
			self.slides.push(self._parseNode(this));
		});
		...
		if(self.st.arrowsNav) {
			...
			self._arrowLeft = container.children('.'+rArr+'Left').click(function() {
				self.prev();
			});
			self._arrowRight = container.children('.'+rArr+'Right').click(function() {
				self.next();
			});
	...
	RoyalSlider.prototype = {
		...
		addSlide: function(htmltext, index) {
			var self = this;
			// TODO
			// var parsedSlide = self._parseNode(htmltext);
			// self.slides.push(parsedSlide);
			// self.slidesJQ.push('<div style="'+ (self._isMove ? 'position: absolute;' : 'z-index: 0; display:none; opacity: 0; position: absolute;  left: 0; top: 0;' ) +'" class="rsSlide"></div>');

			// self.numSlides++;

			// self.ev.trigger('rsOnAddSlide', [parsedSlide, index])
